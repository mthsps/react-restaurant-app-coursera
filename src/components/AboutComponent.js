...
import { FadeTransform } from 'react-animation-components';
 ...
 function RenderLeader ( {leader} ) {
        return (
            ...
            <div className='row'>
                <Media key={leader.id}>
                    <img className="mr-5" src={baseUrl + leader.image} alt={leader.name}/>
                        <Media body>
                            <Media heading className="mt-0 mb-1">{leader.name}</Media>
                            <p>{leader.designation}</p>
                            <p className="mb-5">{leader.description}</p>
                        </Media>
                </Media>
            </div>
            </div>
...
function About(props) {
    if (props.leaders.isLoading) {
        ...
        var leaders = props.leaders.leaders.map((leader) => {
                return (
                    <div>
